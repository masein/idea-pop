...
async fn seed_challenges(pool: &PgPool) -> anyhow::Result<()> {
  ...
  {"step":"skill","instructions":"Learn about FEATURES — the little clues a classifier uses (pointy ears? whiskers? barks? round or oval?). UNPLUGGED: make a paper 'feature-checklist' classifier — a card that scores each mystery picture on 4 features and votes cat or dog. PLUGGED (optional): open Google Teachable Machine, make two classes, and get ready to train it with about 20 images each.","skill_refs":[],"hints":["A feature is a clue you can SEE — like pointy ears or a round shape. Pick 4 clues that are different between your two categories.", "The give-away: score each mystery picture 1 point per matching clue — whichever team scores more points wins the guess!"]},
  ...
  {"step":"skill","instructions":"Learn how one-sided data creates bias. UNPLUGGED: build a card classifier trained only on red apples, then test it on a green apple and a red ball — watch it fail. PLUGGED (optional): in Teachable Machine, train an 'apple' class using only red apples, then test green apples; then RETRAIN with varied apples and compare.","skill_refs":[],"hints":["Train with ONLY red apples on purpose — then test a green apple and watch closely what happens.", "The give-away: the machine never learned 'apple' — it secretly learned 'red'. That hidden shortcut is the bias!"]},
